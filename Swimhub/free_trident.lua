...
local function getfile(name)
	local repo = 'https://raw.githubusercontent.com/1337kat/V5/main/Swimhub/'
...
local function readswimhubfile(file)
	if not isswimhubfile(file) then
		return false
	end
...
do
	...
	if not isfolder('swimhub/new') then
		makefolder('swimhub/new')
	end
	if not isfolder('swimhub/new/files') then
		makefolder('swimhub/new/files')
	end
...
cheat.Library, cheat.Toggles, cheat.Options =
	loadswimhubfile('library_main.lua')()
...
local ui = {
	window = cheat.Library:CreateWindow({
		Title = string.format(SWG_Title, SWG_Version, SWG_FullName),
		Center = true,
		AutoShow = true,
		TabPadding = 8,
	}),
...
repeat
	if
		not pcall(function()
			...
			trident.original_model =
				game:GetService('ReplicatedStorage').Shared.entities.Player.Model
			trident.tcp = LocalPlayer.TCP
		end)
	then
		task.wait(0.5)
	end
until trident.middlepart and trident.original_model and trident.tcp
...
LPH_NO_VIRTUALIZE(function()
	...
	local container =
		Instance.new('Folder', game:GetService('CoreGui').RobloxGui)
	esp_table = {
		__loaded = false,
		main_settings = {
			textSize = 15,
	...
	local function getBoundingBox(parts)
		...
		for i = 1, #parts do
			...
			min = _Vector3zeromin(
				min or cframe.Position,
				(cframe - size * 0.5).Position
			)
			max = _Vector3zeromax(
				max or cframe.Position,
				(cframe + size * 0.5).Position
			)
		end
	...
	local function calculateCorners(cframe, size)
		...
		for i = 1, #VERTICES do
			corners[i] =
				worldToScreen((cframe + size * 0.5 * VERTICES[i]).Position)
		end
	...
	for i, v in
		game:GetService('ReplicatedStorage').Shared.entities:GetChildren()
	do
	...
	local function identify_model(model)
		...
		if
			_FindFirstChildOfClass(model, 'MeshPart')
			and _FindFirstChildOfClass(model, 'MeshPart').MeshId
				== 'rbxassetid://12939036056'
		then
		...
		for name, entity in enttiyidentification do
			if
				entity.Color == primpart.Color
				and entity.Material == primpart.Material
				and entity.CollisionGroup == primpart.CollisionGroup
			then
	...
	local function create_esp(model)
		if
			model
			and _FindFirstChild(model, 'Head')
			and _FindFirstChild(model, 'LowerTorso')
		then
			local settings = esp_table.settings.enemy
			loaded_plrs[model] = {
				obj = {
					box_fill = esp.create_obj(
						'Square',
						{ Filled = true, Visible = false }
					),
					box_outline = esp.create_obj('Square', {
						Filled = false,
						Thickness = 3,
						Visible = false,
						ZIndex = -1,
					...
					box = esp.create_obj(
						'Square',
						{ Filled = false, Thickness = 1, Visible = false }
					),
					realname = esp.create_obj(
						'Text',
			...
			for required, _ in next, skeleton_order do
				loaded_plrs[model].obj['skeleton_' .. required] =
					esp.create_obj('Line', { Visible = false })
			end
			...
			function plr:forceupdate()
				...
				cham.DepthMode = settings.chams_visible_only
					...
					or not settings.chams_visible_only
			...
			plr.connection = cheat.utility.new_renderstepped(function(delta)
				if not (settings.enabled and head and character) then
				...
				do
					...
					realname.Position = (corners.topLeft + corners.topRight)
							* 0.5
						- Vector2.yAxis * realname.TextBounds.Y
						- _Vector2new(0, 2)
				end

				do
					local bottom = (corners.bottomLeft + corners.bottomRight)
						* 0.5
					dist.Text = tostring(mathround(distance)) .. ' studs'
					dist.Position = bottom
					weapon.Text = 'unknown'
					weapon.Position = bottom
						+ (
							dist.Visible
								and Vector2.yAxis * dist.TextBounds.Y - _Vector2new(
									0,
									2
								)
							or Vector2.zero
				...
				if settings.skeleton then
					for _, part in next, character:GetChildren() do
						...
						local parent_part = skeleton_order[part.Name]
							and _FindFirstChild(
								character,
								skeleton_order[part.Name]
							)
						if skeletonobj and parent_part then
							local part_position, _ =
								_WorldToViewportPoint(camera, part.Position)
							local parent_part_position, _ =
								_WorldToViewportPoint(
									camera,
									parent_part.CFrame.p
								)
							skeletonobj.From =
								_Vector2new(part_position.X, part_position.Y)
							skeletonobj.To = _Vector2new(
								parent_part_position.X,
			...
			loaded_plrs[model] = {
				obj = {
					name = esp.create_obj(
						'Text',
						{ Center = true, Visible = false, Text = espname }
					),
				},
			}
			local plr = loaded_plrs[model]
			...
			plr.connection = cheat.utility.new_heartbeat(function(delta)
				local plr = loaded_plrs[model]
				if
					not (settings.enabled and mainpart and allowedobjs[espname])
				then
					return plr:togglevis(false)
				end
...
do
	...
	local hbc, original_size, hbsize =
		nil, trident.original_model.Head.Size, _Vector3new(0.5, 1, 0.5)
	local dynamic, alwayshead = false, false
	local hitboxheadsizex, hitboxheadsizey, hitboxheadtransparency, cancollide =
		10, 10, 0.5, false
	local function addtovc(obj)
		...
		if
			not obj:FindFirstChild('Head')
			and not obj:FindFirstChild('LowerTorso')
		then
			return
		end
	...
	local function removefromvc(obj)
		if not validcharacters[obj] then
			return
		end
	...
	hbb:AddToggle('hitbox_cancollide', {
		Text = 'can collide',
		Default = false,
		Callback = function(v)
			cancollide = v
	...
	hbb:AddSlider('hitbox_head_transparency', {
		Text = 'transparency',
		Default = 0.5,
		Min = 0,
		Max = 1,
	...
	hbb:AddSlider('hitbox_head_size_x', {
		Text = 'size x',
		Default = 10,
		Min = 1,
		Max = 15,
	...
	hbb:AddSlider('hitbox_head_size_y', {
		Text = 'size y',
		Default = 10,
		Min = 1,
		Max = 15,
...
do
	...
	espb:AddSlider('espskeletontransparency', {
	...
	espb:AddLabel('chams outline color')
...
do
	...
	espb:AddDropdown('objectespfont', {
		Values = { 'UI', 'System', 'Plex', 'Monospace' },
		Default = 1,
		Multi = false,
		Text = 'esp font',
		Tooltip = 'select font',
		Callback = function(Value)
			cheat.EspLibrary.main_object_settings.textFont =
				Drawing.Fonts[Value]
			cheat.EspLibrary.icaca()
		end,
	})
	espb:AddSlider('objectespfontsize', {
		Text = 'esp font size',
		Default = 13,
		Min = 1,
		Max = 30,
	...
	espb:AddDropdown('objectespallowed', {
		Values = {
			'ATV',
			'Stone',
			'Nitrate',
		...
		Multi = true,
		Text = 'objects',
		Tooltip = 'select objects thats gonna show up',
		Callback = function(Value)
...
do
	...
	local watermarktoggle = watermarktab:AddToggle('watermark_enabled', {
		Text = 'enabled',
		Default = false,
		Callback = function(first)
			enabled = first
	...
	watermarktoggle:AddColorPicker('watermark_color1', {
		Default = Color3.new(),
		Title = canuseimages and 'color left' or 'color',
		Transparency = 0,
		Callback = function(Value)
			color1 = Value
			if not canuseimages then
				color2 = Value
			end
		end,
	})
	if canuseimages then
		watermarktoggle:AddColorPicker('watermark_color2', {
			Default = Color3.new(),
			Title = 'color right',
			Transparency = 0,
			Callback = function(Value)
	...
	watermarktab:AddToggle('watermark_rainbowcolor', {
		Text = 'rainbow',
		Default = false,
		Callback = function(first)
			dynamic = first
	...
	if canuseimages then
		rawget(gradr, '_frame').Image =
			getcustomasset('swimhub/new/files/grad90r.png')
	end
	if canuseimages then
		rawget(gradl, '_frame').Image =
			getcustomasset('swimhub/new/files/grad90l.png')
	end
	...
	cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
		...
		watertext = ('swimhub %s | %s | %s | %s fps'):format(
			SWG_Version,
			SWG_ShortName,
			os.date('%b %d %Y'),
			tostring(finalfps)
		...
		textbackground.Size =
			Vector2.new(text.TextBounds.X + 7, text.TextBounds.Y + 6)
		textbackground.Position = waterpos + Vector2.new(2, 2)
		gradbackground.Size =
			Vector2.new(text.TextBounds.X + 11, text.TextBounds.Y + 10)
...
do
	...
	WorldTab:AddSlider('timechanger', {
		Text = 'time changer',
		Default = mathround(Lighting.ClockTime),
		Min = 0,
		Max = 24,
		Rounding = 1,
		Compact = false,
...
do
	...
	cursor.sussy = false
	CrosshairTab:AddDropdown('espfont', {
		Values = { 'UI', 'System', 'Plex', 'Monospace' },
		Default = 1,
		Multi = false,
		Text = 'esp font',
	...
	local math_cos, math_atan, math_pi, math_sin =
		math.cos, math.atan, math.pi, math.sin
	...
	cheat.utility.new_renderstepped(LPH_NO_VIRTUALIZE(function(delta)
		if cursor.Enabled then
			...
			if cursor.CustomPos then
				pos = cursor.Position
			else
				pos = _Vector2new(Mouse.X, Mouse.Y + GuiInset.Y)
			end
			if cursor.rainbow then
				color = Color3.fromHSV(rainbow, 1, 1)
			else
				color = cursor.Color
			end
			if transp <= 1.5 + cursor.Text.LogoFadingOffset and not reverse then
				transp = transp
					+ (((cursor.Speed == 0 and 1 or cursor.Speed) * 10) * delta)
				if transp >= 1.5 + cursor.Text.LogoFadingOffset then
					setreverse(true)
				end
			elseif reverse then
				transp = transp
					- (((cursor.Speed == 0 and 1 or cursor.Speed) * 10) * delta)
				if transp <= 0 - cursor.Text.LogoFadingOffset then
					setreverse(false)
				end
			end
			logotext.Position =
				_Vector2new(pos.X, (pos + _Vector2new(0, cursor.Radius + 5)).Y)
			...
			if cursor.sussy then
				...
				if rotationdegree >= 90 then
					rotationdegree = 0
				end

				for i = 1, 4 do
					local p_0 = (
						a * math_sin(DEG2RAD(rotationdegree + (i * 90)))
					)
					local p_1 = (
						a * math_cos(DEG2RAD(rotationdegree + (i * 90)))
					)
					local p_2 = (
						(a / math_cos(gamma))
						* math_sin(
							DEG2RAD(rotationdegree + (i * 90) + RAD2DEG(gamma))
						)
					)
					local p_3 = (
						(a / math_cos(gamma))
				...
				rotationdegree = rotationdegree
					+ ((cursor.Speed * frametime) * 1000)
			else
				angle = (
					cursor.Speed == 0 and 0
					or angle + ((cursor.Speed * 10) * delta)
				)
							...
							pos.X
								+ (
									...
									* (
										...
										- (
											cursor.TheGap
												and ((cursor.Radius - 20 + 1) / cursor.Gap)
											or (
												(cursor.Radius - 20 + 1)
												- cursor.Gap
											)
							...
							pos.X
								+ (
									...
									* (
										...
										- (
											cursor.TheGap
												and ((cursor.Radius - 20) / cursor.Gap)
											or (
												(cursor.Radius - 20)
												- cursor.Gap
											)
...
do
	...
	mvb:AddToggle('speedhack_forcesprint', {
		Text = 'forcesprint',
		Default = false,
		Callback = function(first)
			forcesprint = first
	...
	mvb:AddToggle('speedhack_enabled', {
	...
	cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
		if
			bhop_enabled
			and middle
			and _IsKeyDown(UserInputService, Enum.KeyCode.C)
			and _IsKeyDown(UserInputService, Enum.KeyCode.LeftShift)
		then
			local cameralook = Camera.CFrame.LookVector
			cameralook = _Vector3new(cameralook.X, 0, cameralook.Z)
			local direction = Vector3.zero
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.W)
					and direction + cameralook
				or direction
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.S)
					and direction - cameralook
				or direction
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.D)
					and direction + _Vector3new(-cameralook.Z, 0, cameralook.X)
				or direction
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.A)
					and direction + _Vector3new(cameralook.Z, 0, -cameralook.X)
				or direction
	...
	local findcar = function()
		...
		for i, v in pairs(workspace:GetChildren()) do
			if v:FindFirstChild('Seat') and v:FindFirstChild('Frame') then
				if
					(v.Frame.Position - trident.middlepart.Position).Magnitude
					< dist
				then
					car = v
	...
	cheat.utility.new_renderstepped(LPH_JIT_MAX(function(delta)
		...
		then
			...
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.A)
					and direction + _Vector3new(cameralook.Z, 0, -cameralook.X)
				or direction
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.V)
					and direction + Vector3.yAxis
				or direction
			direction = _IsKeyDown(UserInputService, Enum.KeyCode.B)
					and direction - Vector3.yAxis
				or direction
...
loadswimhubfile('chat_spam.lua')(
	cheat.Library,
	ui.box.misc:AddTab('chat spam'),
	function(word)
...
ui.box.themeconfig:AddToggle('keybindshoww', {
	...
	Default = false,
	Callback = function(first)
		cheat.Library.KeybindFrame.Visible = first
	end,
})
